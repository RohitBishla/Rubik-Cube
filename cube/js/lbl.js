...
solver.logic = function (cube) {
  ...
  function Execute(steps) {
    for (let i = 0; i < steps.length; i++) {
      ...
      if (step != "") {
        ...
        if (i != steps.length - 1) {
          typewriter
            .typeString(stepName[i] + " is already solved.")
            .pauseFor(500)
            .deleteAll()
            .start();
        } else
          typewriter.typeString(stepName[i] + " is already solved.").start();
      }
      console.log(step);
  ...
  function getCubeState() {
    cubeState = {
      ...
      dl:
        cube.down.south.down.color.name[0] + cube.down.south.left.color.name[0],
      df:
        cube.down.west.down.color.name[0] + cube.down.west.front.color.name[0],
      dr:
        cube.down.north.down.color.name[0] +
        cube.down.north.right.color.name[0],
      ...
      lf:
        cube.front.west.left.color.name[0] +
        cube.front.west.front.color.name[0],
      fr:
        cube.front.east.front.color.name[0] +
        cube.front.east.right.color.name[0],
      rb:
        cube.back.north.right.color.name[0] +
        cube.back.north.back.color.name[0],
      bl:
        cube.back.south.back.color.name[0] + cube.back.south.left.color.name[0],

      dlf:
        cube.down.southWest.down.color.name[0] +
        cube.down.southWest.left.color.name[0] +
        cube.down.southWest.front.color.name[0],
      dfr:
        cube.down.northWest.down.color.name[0] +
        cube.down.northWest.front.color.name[0] +
        cube.down.northWest.right.color.name[0],
      drb:
        cube.down.northEast.down.color.name[0] +
        cube.down.northEast.right.color.name[0] +
        cube.down.northEast.back.color.name[0],
      dbl:
        cube.down.southEast.down.color.name[0] +
        cube.down.southEast.back.color.name[0] +
        cube.down.southEast.left.color.name[0],

      ulf:
        cube.up.southWest.up.color.name[0] +
        cube.up.southWest.left.color.name[0] +
        cube.up.southWest.front.color.name[0],
      ufr:
        cube.up.southEast.up.color.name[0] +
        cube.up.southEast.front.color.name[0] +
        cube.up.southEast.right.color.name[0],
      ubl:
        cube.up.northWest.up.color.name[0] +
        cube.up.northWest.back.color.name[0] +
        cube.up.northWest.left.color.name[0],
      urb:
        cube.up.northEast.up.color.name[0] +
        cube.up.northEast.right.color.name[0] +
        cube.up.northEast.back.color.name[0],
    };

    var color_l = cubeState["l"],
      color_r = cubeState["r"],
      color_f = cubeState["f"],
      color_b = cubeState["b"];
      ...
      if (s.k.indexOf("d") != -1) {
        ...
        if (s.k[1] == block_pos[1]) {
          if (s.v[0] == block_color[0])
            ...
            cubeState[block_pos[0] + nextFace[s.k[1]]] !=
            cubeState[block_pos[0]] + cubeState[nextFace[s.k[1]]]
          )
            ...
            exp =
              ...
              nextFace[s.k[1]] +
              s.k[1].toUpperCase() +
              nextFace[s.k[1]].toUpperCase();
        } else exp = "U";
      } else {
        if (s.v[0] == block_color[0]) {
          if (s.k[1] == block_pos[1]) exp = s.k[1];
          else if (
            cubeState[block_pos[0] + s.k[1]] !=
            cubeState[block_pos[0]] + cubeState[s.k[1]]
          )
          ...
          if (s.k[0] == block_pos[1]) exp = s.k[0].toUpperCase();
            ...
            cubeState[block_pos[0] + s.k[0]] !=
            cubeState[block_pos[0]] + cubeState[s.k[0]]
          )
            exp = s.k[0];
  ...
  function SECOND_LAYER_SINGLE(block_pos, block_color) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (s.k.indexOf("u") != -1) {
        ...
        if (s.v[0] == cubeState[s.k[0]] && s.v[1] == cubeState[s.k[1]])
          return exp_log;
        else
          exp =
            "u" +
  ...
  function FIRST_LAYER_CORNERS() {
    ...
    // );
    let order = "";
    order += FIRST_LAYER_CORNERS_SINGLE(
      "dlf",
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (let j = i + 1; j < i + 4; j++) {
        ...
        if (nextColor[lastc[0]] == lastc[1]) {
          if (
            nextc.indexOf(lastc[1]) != -1 &&
            nextc.indexOf(nextColor[lastc[1]]) != -1
          )
            times += 1;
          else break;
        } else {
          if (
            nextc.indexOf(lastc[0]) != -1 &&
            nextc.indexOf(nextColor[lastc[0]]) != -1
          )
            times += 1;
          else break;
        }
  ...
  function THIRD_LAYER_EDGES() {
    ...
    for (let i = 0; i < 3; i++) {
      let exp = "";
      const s =
        cubeState.uf[1] + cubeState.ur[1] + cubeState.ub[1] + cubeState.ul[1];
  ...
  function compress(order) {
    for (let i = 0; i < 10; i++) {
      order = order.replace(
        /uU|Uu|dD|Dd|lL|Ll|fF|Ff|rR|Rr|bB|Bb|yY|Yy|uuuu|dddd|llll|ffff|rrrr|bbbb|yyyy|UUUU|DDDD|LLLL|FFFF|RRRR|BBBB|YYYY/g,
        ""
      );
